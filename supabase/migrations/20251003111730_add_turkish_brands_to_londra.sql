/*
  # Add Turkish Brands to Londra

  1. Changes
    - Adds standard Turkish brands to Londra (slug: londra)
    - Same gold standard as Berlin and Paris
*/
...
BEGIN
  ...
  -- Delete existing Turkish brands for Londra
  DELETE FROM source_country_brands 
  WHERE country_code = 'TR' 
  AND mapping_id IN (
    SELECT ccm.id FROM city_category_mappings ccm WHERE ccm.city_id = v_city_id
  );
